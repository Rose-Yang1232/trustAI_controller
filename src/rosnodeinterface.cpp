  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"TrustAI_ros\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(TrustAI_ros_M, (NULL));
        TrustAI_ros_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(TrustAI_ros_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(TrustAI_ros_M)
                == (NULL)) || rtmGetStopRequested(TrustAI_ros_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(TrustAI_ros_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
